// Libros Fuentes:
// https://www.amazon.es/Unix-System-Release-Programmers-Reference/dp/0139470298
...
#define IS_ALIGNED(x) ((((uint64_t)(x))&(PAGESIZE-1)) == 0) // Comprueba si está alineado obteniendo el resto del máximo
...
// Traduce de página a bitfields, y obtiene que es lo que va a hacer cada cosa


int init_memory() {
    static struct chunk_data biggest_avail_chunk;
    if (biggest_avail_chunk.size > 0) return 0; // Si >0 significa que ya hemos ejecutado init_memory y petaría

    uint64_t total_memory = get_total_memory();
    //uint64_t free_memory = get_free_memory();
    uint64_t total_pages = total_memory >> 12; // Divide entre 4096 y pasa de bytes a páginas totales

    iterate_memory(init_memory_cb, (void*)&biggest_avail_chunk);
    //Aquí tenenmos el chunk de memoria disponible más grande

    printf("Tenemos %lld bytes de memoria\n", total_memory); // %llx = 64bits en hexadecimal
    //printf("Tenemos %lld bytes de memoria libre\n", free_memory);
    printf("Tenemos %lld bytes de memoria disponible\n", biggest_avail_chunk.size);
    ...
    // Sanity test para asegurar que todo está bien
    for (int i = 0; i < 100; i++) {
        for (int j = 0; j < 8; j++) {
            if (j % 2 == 1) {
